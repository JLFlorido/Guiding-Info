"""Run PINN to solve Burger's Equation in one spatial dimension and time. 
adaptive.py. Starting with a specified initial distribution, this code then uses the specified guiding information (input1) to guide adaptive
resampling of collocation points for a specified number of loops.

Usage:
    adaptive.py [--k=<hyp_k>] [--c=<hyp_c>] [--N=<NumDomain>] [--L=<NumResamples> ] [--IM=<InitialMethod>] [--DEP=<depth>] [--INP1=<input1>]
    adaptive.py -h | --help
Options:
    -h --help                   Display this help message
    --k=<hyp_k>                 Hyperparameter k, specifying resampling behaviour [default: 1]
    --c=<hyp_c>                 Hyperparameter c, specifying resampling behaviour [default: 1]
...
def gen_testdata(): # This function opens the ground truth solution. Adjust path directory to location of .npz file.
    data = np.load("src/burgers/npz/Burgers.npz")
...
def main(k=1, c=1, NumDomain=2000, NumResamples=100, method="Random", depth=3, input1="residual"): # Main Code
    ...
    def dpde_dxt(x,y): #residual wrt x and y
        dy_x = dde.grad.jacobian(y, x, i=0, j=0)
        dy_t = dde.grad.jacobian(y, x, i=0, j=1)
        dy_xx = dde.grad.hessian(y, x, i=0, j=0)
        pde = dy_t + y * dy_x - 0.01 / np.pi * dy_xx    
    ...
    X_test, y_true = gen_testdata() # Read in ground truth solution. (25600,2) coordinates and corresponding (25600,1) values of u.

    # Describe the problem using deepxde method.
    ...
    def output_transform(x, y): # Output transformation to automatically satisfy BC
    ...
    model.train(display_every=250) # As specified at beginning, this is limited to 1000 steps.
    ...
    for i in range(NumResamples): # Resampling loop begins. 100 is default
        X = geomtime.random_points(100000) # Dense grid on which to read guiding information
        ...
        # Calculate probability distribution function from guiding information
        err_eq = np.power(Y, k) / np.power(Y, k).mean() + c 
        err_eq_normalized = (err_eq / sum(err_eq))[:, 0]
        # And accordingly select new points.
        ...
        model.compile("adam", lr=0.001)
        model.train(epochs=1000, display_every=10000)
        model.compile("L-BFGS")
    ...
    # To save loss curve, final point distribution and final prediction.
    # dde.saveplot(losshistory, train_state, issave=False, isplot=False, 
    #              loss_fname=f"adaptive_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_loss_info.dat", 
    #              train_fname=f"adaptive_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalpoints.dat", 
    #              test_fname=f"adaptive_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalypred.dat",
...
if __name__ == "__main__":
    ...
    error_final, time_taken = main(c=c, k=k, NumDomain=NumDomain,NumResamples=NumResamples,method=method, depth=depth, input1=input1) # Run main, recording time taken and final accuracy.
    ...
    output_dir = "/results/raw"  # Replace with your desired output directory path
    error_final_fname = f"adaptive_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"adaptive_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_time_taken.txt"
